...
import { PositionArray } from '../../src/geometry/position-array.js'
...
describe('PositionArray', () => {
  ...
  describe('dimension', () => {
    it('returns 2 for 2D position', () => {
      expect(PositionArray([[1, 2]]).dimension).toBe(2)
    })

    it('returns 3 for 3D position', () => {
      expect(PositionArray([[1, 2, 3]]).dimension).toBe(3)
    })
  })
  ...
  describe('at', () => {
    ...
    it('returns a Position with altitude in 3D', () => {
      const ca = PositionArray([[1.5, 48.8, 100]])
  ...
  describe('Symbol.iterator', () => {
    it('iterates over all points', () => {
      const ca = PositionArray([[1, 2], [3, 4]])
      const result = [...ca]
  ...
  describe('map', () => {
    ...
    it('returns [] if invalid', () => {
      expect(PositionArray([[null, 2]]).map(c => c)).toEqual([])
    })
  })
  ...
  describe('toArray', () => {
    ...
    it('returns [] if invalid', () => {
      expect(PositionArray([[null, 2]]).toArray()).toEqual([])
    })
  })

  describe('toGeoJSON', () => {
    it('returns coordinates in 2D', () => {
      const ca = PositionArray([[1.5, 48.8]])
    ...
    it('returns { coordinates: [] } if invalid', () => {
      expect(PositionArray([[null, 2]]).toGeoJSON()).toEqual({ coordinates: [] })
    })
  })

  describe('bbox', () => {
    it('computes 2D bbox with correct min/max', () => {
      const ca = PositionArray([[1, 10], [5, 20], [3, 15]])
      const bb = ca.bbox()
    ...
    it('computes 3D bbox with correct min/max', () => {
      const ca = PositionArray([[1, 10, 0], [5, 20, 100]])
      const bb = ca.bbox()
      expect(bb).not.toBeNull()
    ...
    it('returns null if invalid', () => {
      expect(PositionArray([[null, 2]]).bbox()).toBeNull()
    })

    it('returns null if empty', () => {
      expect(PositionArray().bbox()).toBeNull()
    })
  })

  describe('centroid', () => {
    it('computes 2D centroid', () => {
      const ca = PositionArray([[0, 0], [2, 4]])
      const c = ca.centroid()
    ...
    it('computes 3D centroid', () => {
      const ca = PositionArray([[0, 0, 0], [2, 4, 100]])
      const c = ca.centroid()
      expect(c.longitude).toBe(1)
      expect(c.latitude).toBe(2)
    ...
    it('returns null if invalid', () => {
      expect(PositionArray([[null, 2]]).centroid()).toBeNull()
    })

    it('returns null if empty', () => {
      expect(PositionArray().centroid()).toBeNull()
    })
  })
  ...
  describe('truncate', () => {
    it('truncates all points', () => {
      ...
      const ca = PositionArray([[1.123456789, 48.987654321]])
      ca.truncate(2)
