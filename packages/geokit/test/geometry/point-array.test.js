...
import { PointArray } from '../../src/geometry/point-array.js'
...
describe('PointArray', () => {
  describe('constructor', () => {
    ...
    it('parses an array of 2D arrays', () => {
      const ca = PointArray([[1.5, 48.8], [2.3, 43.6]])
    ...
    it('parses an array of 3D arrays', () => {
      const ca = PointArray([[1.5, 48.8, 100], [2.3, 43.6, 200]])
      expect(ca.dimension).toBe(3)
    })

    it('parses an array of objects with lon/lat', () => {
      const ca = PointArray([{ lon: 1.5, lat: 48.8 }, { lon: 2.3, lat: 43.6 }])
      expect(ca.isValid()).toBe(true)
      expect(ca.dimension).toBe(2)
    })

    it('parses an array of objects with longitude/latitude', () => {
      const ca = PointArray([{ longitude: 1.5, latitude: 48.8 }])
      expect(ca.isValid()).toBe(true)
    })

    it('detects altitude via alt', () => {
      const ca = PointArray([{ lon: 1, lat: 2, alt: 50 }])
      expect(ca.dimension).toBe(3)
    })

    it('detects altitude via z', () => {
      const ca = PointArray([{ lon: 1, lat: 2, z: 50 }])
      expect(ca.dimension).toBe(3)
    })

    it('marks isValid() as false if longitude is missing', () => {
      const ca = PointArray([[null, 48.8]])
      expect(ca.isValid()).toBe(false)
    })

    it('marks isValid() as false if latitude is not a number', () => {
      const ca = PointArray([{ lon: 1.5, lat: 'oops' }])
      expect(ca.isValid()).toBe(false)
    })

    it('marks isValid() as false if altitude is not a number in 3D', () => {
      const ca = PointArray([[1, 2, 100], [3, 4, 'bad']])
      expect(ca.isValid()).toBe(false)
    })
  })

  describe('dimension', () => {
    it('returns 2 for 2D position', () => {
      expect(PointArray([[1, 2]]).dimension).toBe(2)
    })

    it('returns 3 for 3D position', () => {
      expect(PointArray([[1, 2, 3]]).dimension).toBe(3)
    })
  })

  describe('at', () => {
    it('returns a Position at the given index', () => {
      const ca = PointArray([[1.5, 48.8], [2.3, 43.6]])
      const c = ca.at(1)
    ...
    it('returns a Position with altitude in 3D', () => {
      const ca = PointArray([[1.5, 48.8, 100]])
  ...
  describe('map', () => {
    ...
    it('returns [] if invalid', () => {
      expect(PointArray([[null, 2]]).map(c => c)).toEqual([])
    })
  })
  ...
  describe('toArray', () => {
    ...
    it('returns [] if invalid', () => {
      expect(PointArray([[null, 2]]).toArray()).toEqual([])
    })
  })

  describe('toGeoJSON', () => {
    it('returns coordinates in 2D', () => {
      const ca = PointArray([[1.5, 48.8]])
    ...
    it('returns { coordinates: [] } if invalid', () => {
      expect(PointArray([[null, 2]]).toGeoJSON()).toEqual({ coordinates: [] })
    })
  })

  describe('bbox', () => {
    it('computes 2D bbox with correct min/max', () => {
      const ca = PointArray([[1, 10], [5, 20], [3, 15]])
      const bb = ca.bbox()
    ...
    it('computes 3D bbox with correct min/max', () => {
      const ca = PointArray([[1, 10, 0], [5, 20, 100]])
      const bb = ca.bbox()
      expect(bb).not.toBeNull()
    ...
    it('returns null if invalid', () => {
      expect(PointArray([[null, 2]]).bbox()).toBeNull()
    })

    it('returns null if empty', () => {
      expect(PointArray().bbox()).toBeNull()
    })
  })

  describe('centroid', () => {
    it('computes 2D centroid', () => {
      const ca = PointArray([[0, 0], [2, 4]])
      const c = ca.centroid()
    ...
    it('computes 3D centroid', () => {
      const ca = PointArray([[0, 0, 0], [2, 4, 100]])
      const c = ca.centroid()
      expect(c.longitude).toBe(1)
      expect(c.latitude).toBe(2)
    ...
    it('returns null if invalid', () => {
      expect(PointArray([[null, 2]]).centroid()).toBeNull()
    })

    it('returns null if empty', () => {
      expect(PointArray().centroid()).toBeNull()
    })
  })
  ...
  describe('truncate', () => {
    it('truncates all points', () => {
      ...
      const ca = PointArray([[1.123456789, 48.987654321]])
      ca.truncate(2)
