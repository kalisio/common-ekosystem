...
describe('string.removeDiacritics', () => {
  it('removes diacritics from a string', () => {
    expect(string.removeDiacritics('héllo wörld')).toBe('hello world')
  })

  it('handles uppercase diacritics', () => {
    expect(string.removeDiacritics('HÉLLO')).toBe('HELLO')
  })

  it('handles all diacritic families', () => {
    expect(string.removeDiacritics('àáäâã éëèê íïìî óöòõô üúùû ç'))
      .toBe('aaaaa eeee iiii ooooo uuuu c')
  })

  it('returns the string unchanged if no diacritics', () => {
    expect(string.removeDiacritics('hello')).toBe('hello')
  })

  it('returns an empty string unchanged', () => {
    expect(string.removeDiacritics('')).toBe('')
  })

  it('throws if str is not a string', () => {
    expect(() => string.removeDiacritics(123)).toThrow('str must be a string')
  })
})
